 ...
 *
 * --------------------------------------------------------------------------
 * $Id: BeanBusinessMethodInvokerImpl.java 5369 2010-02-24 14:58:19Z benoitf $
 * --------------------------------------------------------------------------
 */
 ...
 */
public class BeanBusinessMethodInvokerImpl implements EZBInterceptorInvoker {

    /**
     * Logger.
     */
    private static final Log LOGGER = LogFactory.getLog(BeanBusinessMethodInvokerImpl.class);
    ...
    public Object invoke(final EasyBeansInvocationContext invocationContext, final EZBInterceptorManager interceptorManager)
        ...
        DynamicInvocationContextImpl dynamicInvocationContext = (DynamicInvocationContextImpl) invocationContext;


        // Call of the interceptor
        LOGGER.debug("Calling bean with invocationContext ''{0}'' on bean ''{1}'' with parameters ''{2}''", invocationContext,
                invocationContext.getTarget(), Arrays.asList(dynamicInvocationContext.getInternalParameters()));

        // If accessible, call it directly
        if (isAccessible) {
            return invoke(dynamicInvocationContext);
        }
     ...
     * @param dynamicInvocationContext the invocation context given as an argument of the method
     * @return the result of the invocation
     * @throws Exception if invocation fails
     */
   private Object invoke(final DynamicInvocationContextImpl dynamicInvocationContext) throws Exception {
       try {
           Object[] parameters = dynamicInvocationContext.getInternalParameters();
           if (parameters == null || parameters.length == 0) {
               return this.method.invoke(dynamicInvocationContext.getTarget());
           }
           return this.method.invoke(dynamicInvocationContext.getTarget(), parameters);
