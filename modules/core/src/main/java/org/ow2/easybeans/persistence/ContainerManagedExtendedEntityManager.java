 ...
 */
public class ContainerManagedExtendedEntityManager implements EZBExtendedEntityManager {
     ...
     * Counter.
     */
    private AtomicInteger usage = null;

    /**
     * Wrap an entity manager.
     * @param entityManager the wrapped entity manager
     */
    public ContainerManagedExtendedEntityManager(final EntityManager entityManager) {
    ...
    public void closeInternal() {
        ...
        // Last count (so we should be in the last remove() call on a SFSB)
        // so we can close the entity Manager.
        if (newCount == 0) {
