...
/**
 * This class resolve business methods for a ManagedBean class metadata.
 *
...
public class ManagedBeanBusinessMethodResolver {
     ...
     * A business method is a public method that is not a constructor not a lifecycle nor an AroundInvoke method.
     *
    ...
    public static void resolve(final EasyBeansEjbJarClassMetadata classAnnotationMetadata) {
        for (EasyBeansEjbJarMethodMetadata methodData : classAnnotationMetadata.getMethodMetadataCollection()) {
            if ((Opcodes.ACC_PUBLIC & methodData.getJMethod().getAccess()) == Opcodes.ACC_PUBLIC) {
                ...
                if (methodData.isLifeCycleMethod() || methodData.isAroundInvoke()) {
                    continue;
                }
        ...
        SessionBeanValidator.validate(classAnnotationMetadata);
    }
}
